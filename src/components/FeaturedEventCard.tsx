...
export function FeaturedEventCard(props: { event: FeaturedEvent; className?: string }) {
  ...
  return (
    <Link
      ...
      className={cn(
        'tv-card group block overflow-hidden p-0 transition-all duration-300 hover:-translate-y-1 hover:bg-[color:var(--card-hover)]',
        props.className,
      )}
    >
      <div className="relative aspect-[16/9] w-full overflow-hidden border-b border-white/10 bg-black/20">
        ...
        {event.badge ? (
          <span className="absolute right-4 top-4 rounded-full border border-white/10 bg-black/60 px-3 py-1.5 text-[11px] font-semibold text-[color:var(--accent)] backdrop-blur tracking-wider">
            {event.badge}
          </span>
        ) : null}
      </div>
      ...
      <div className="p-6">
        ...
        <div className="mt-4">
          ...
          <p className="mt-2 line-clamp-2 text-sm text-[color:var(--ink-muted)]">
            {event.description}
          </p>

          <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1.5 text-xs text-[color:var(--ink-muted)]">
